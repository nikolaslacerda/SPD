...
- [] T004.5 Establish Dependency Injection boundaries for database isolation and state management guardrails for VRAM protection before implementing benchmark execution loops. This ensures benchmark runs do not permanently corrupt the global chat state or inject test data into the main database.
...
- [ ] T026.5 [US1] Refactor VectorStore to receive the dbName via an InjectionToken (e.g., VECTOR_DB_CONFIG). Configure the global provider to use 'takere-db'. Configure the provider within the Benchmark scope to use 'takere-benchmark-db' and force the creation of new VectorStore and RagEngine instances in the benchmark component's providers array.
...
- [ ] T049 [P] Apply SCSS styling using strictly the global variables defined in `src/styles.scss` and `src/app/home/home.component.scss` (e.g., variables for background color, typography, and spacing). Reuse existing utility classes and mixins for buttons, cards, and inputs to ensure visual consistency without introducing hardcoded values ​​(such as hex colors or margins in arbitrary pixel values).
